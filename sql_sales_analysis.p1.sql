SELECT * FROM minimarket_retail.sales_analysis;

### 1.Data cleaning.

-- Record Count: Determine the total number of records in the dataset.

SELECT COUNT(*) FROM minimarket_retail.sales_analysis;

-- Customer Count: Find out how many unique customers are in the dataset.

SELECT COUNT(DISTINCT customer_id) FROM minimarket_retail.sales_analysis;

-- Category Count: Identify all unique product categories in the dataset.

SELECT DISTINCT category FROM minimarket_retail.sales_analysis;

-- Null Value Check: Check for any null values in the dataset and delete records with missing data.

SELECT * FROM minimarket_retail.sales_analysis
WHERE 
    sale_date IS NULL OR sale_time IS NULL OR customer_id IS NULL OR 
    gender IS NULL OR age IS NULL OR category IS NULL OR 
    quantity IS NULL OR price_per_unit IS NULL OR cogs IS NULL;
    
    DELETE FROM minimarket_retail.sales_analysis
WHERE 
    sale_date IS NULL OR sale_time IS NULL OR customer_id IS NULL OR 
    gender IS NULL OR age IS NULL OR category IS NULL OR 
    quantity IS NULL OR price_per_unit IS NULL OR cogs IS NULL;
    
    ### 2. Data Analysis & Findings

-- Developed SQL queries to answer specific business questions:

-- 1.SQL query to retrieve all columns for sales made on '2022-11-05**:

SELECT *
FROM minimarket_retail.sales_analysis
WHERE sale_date = '2022-11-05';

-- 2.SQL query to retrieve all transactions where the category is 'Clothing' and the quantity > 4 in the month of Nov-2022.

SELECT 
  *
FROM minimarket_retail.sales_analysis
WHERE 
    category = 'Clothing'
    AND 
    sale_date BETWEEN '2022-11-01' AND '2022-11-31'
    AND
    quantity >= 4;
    
-- 3.SQL query to calculate the total sales (total_sale) for each category.

SELECT 
    category,
    SUM(total_sale) as net_sale,
    COUNT(*) as total_orders
FROM minimarket_retail.sales_analysis
GROUP BY 1;

-- 4.SQL query to find the average age of customers who purchased items from the 'Beauty' category.

-- Write a SQL query to find the average age of customers who purchased items from the 'Beauty' category.

SELECT
    ROUND(AVG(age), 2) AS avg_age
FROM minimarket_retail.sales_analysis
WHERE category = 'Beauty';

-- 5.SQL query to find all transactions where the total_sale is greater than 1000.

SELECT * FROM minimarket_retail.sales_analysis
WHERE total_sale > 1000;

-- 6.SQL query to find the total number of transactions (transaction_id) made by each gender in each category.

SELECT 
    category,
    gender,
    COUNT(*) as total_trans
FROM minimarket_retail.sales_analysis
GROUP 
    BY 
    category,
    gender
ORDER BY 1;

-- 7.SQL query to calculate the average sale for each month. Find out best selling month in each year.

SELECT
    EXTRACT(YEAR FROM sale_date) AS year,
    EXTRACT(MONTH FROM sale_date) AS month,
    AVG(total_sale) AS avg_sale
FROM minimarket_retail.sales_analysis
GROUP BY year, month
ORDER BY year, month;

-- 8.SQL query to find the top 5 customers based on the highest total sales.

SELECT
    customer_id,
    SUM(total_sale) as total_sales
FROM minimarket_retail.sales_analysis
GROUP BY customer_id
ORDER BY 2 DESC
LIMIT 5;

-- 9.SQL query to find the number of unique customers who purchased items from each category.

SELECT 
    category,    
    COUNT(DISTINCT customer_id) as cnt_unique_cs
FROM minimarket_retail.sales_analysis
GROUP BY category;
-- 10.SQL query to create each shift and number of orders (Example Morning <12, Afternoon Between 12 & 17, Evening >17).

WITH hourly_sale
AS
(
SELECT *,
    CASE
        WHEN EXTRACT(HOUR FROM sale_time) < 12 THEN 'Morning'
        WHEN EXTRACT(HOUR FROM sale_time) BETWEEN 12 AND 17 THEN 'Afternoon'
        ELSE 'Evening'
    END as shift
FROM minimarket_retail.sales_analysis
)
SELECT 
    shift,
    COUNT(*) as total_orders    
FROM hourly_sale
GROUP BY shift;